...
const EditFaculty = () => {
  ...
  useEffect(() => {
    const fetchFacultyDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:2004/api/faculty/all/${id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:2004/api/faculty/update/${id}`, formData);
      if (response.status === 200) {
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 flex justify-center items-center">
        <div className="max-w-lg w-full mt-8 p-6 bg-white shadow-md rounded-lg">
          <h2 className="text-2xl font-bold text-center mb-6 text-indigo-600">Edit Faculty</h2>
